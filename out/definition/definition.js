...
const defaultValueLoader_1 = require("./defaultValueLoader");
class Definitions {
    items;
    constructor(jsonDefinition) {
        let tedDefinition;
        if (jsonDefinition === undefined) {
            jsonDefinition = defaultValueLoader_1.JSON_DEFINITION;
        }
    ...
    toHtml() {
        let htmlBody = "";
        let rootItems = this.items.filter((item) => item.root === true);
    ...
    getConceptDefintion(concept) {
        return this.items.filter((item) => item.name === concept)[0];
    }
    static listDistinctDefinitionDatabases(definition) {
        let databases = Array();
